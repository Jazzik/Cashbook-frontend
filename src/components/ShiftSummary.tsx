...
const ShiftSummary: React.FC<ShiftSummaryProps> = ({
  ...
  // Расчет выручки по всем правилам экономики:
  // 1. Безналичная выручка: терминал минус возвраты по терминалу плюс переводы
  // 2. Наличная выручка: наличные в кассе минус начальное сальдо плюс расходы плюс возвраты наличными минус внесения наличных
  const terminalRevenue = terminal - terminalReturns + terminalTransfer;
  const cashRevenue =
    cashInRegister - initialBalance + expenses + cashReturns - cashDeposits;
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
        <Chip
          label={`Начальное сальдо: ${formatCurrency(initialBalance)}`}
          sx={{
        ...
        />
        <Chip
          label={`Конечное сальдо: ${formatCurrency(finalBalance)}`}
          sx={{
            backgroundColor: "rgba(255,255,255,0.2)",
            color: "white",
            fontWeight: "bold",
          ...
          {submitSuccess ? (
            ...
            <>
              ...
              <Typography variant="body2" color="text.secondary">
                При закрытии смены все данные будут сохранены и отправлены в
                базу. После закрытия данные будут сброшены для новой смены.
              </Typography>
            </>
          )}
        </DialogContent>
